...
namespace NRender {
    ...
    void TRenderer::DrawMap() {
        ...
        for (const auto objectPtr : Engine.GetConstObjects(true)) {
            SDL_Rect objRect;
            objRect.x = mapTopX + static_cast<int>(mapSize * objectPtr->GetPosition().X / Engine.GetWorldSize().X - objSise / 2);
            objRect.y = mapTopY + static_cast<int>(mapSize * objectPtr->GetPosition().Y / Engine.GetWorldSize().Y - objSise / 2);
            objRect.w = objSise;
            objRect.h = objSise;
